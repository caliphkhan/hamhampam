...
if (! function_exists('manageProviderValues')) {
     ...
     * @return void
     */
    function manageProviderValues(string $provider, string $modelKey, string $name): void 
    {
        ...
        foreach ($data as $key => $items) {
            ...
            foreach ($items as $item) {
                if (isset($item['type']) && $item['type'] === 'dropdown') {
                    ...
                    $requestedValue = request()->input($inputKey);
        ...
        foreach ($valuesExist as $field => $exists) {
            if (!$exists) {
                ...
                throw new Exception(__(
                    ":x requested value ':y' for ':z' does not exist. Please contact administration.",
                    [ 'x' => ucfirst($name),'y' => ucfirst($requestedValue), 'z' => ucfirst(str_replace('_', ' ', $field)) ]
                ));
            }
        }
    }
