...
@php
    ...
    $adminTemplateAccess = $isAdmin || ($permissions->hide_template ?? 0) != 1;
    $adminSpeechToTextAccess = $isAdmin || ($permissions->hide_speech_to_text ?? 0) != 1;
    $adminImageAccess = $isAdmin || ($permissions->hide_image ?? 0) != 1;
    $adminCodeAccess = $isAdmin || ($permissions->hide_code ?? 0) != 1;
    $adminTextToSpeechAccess = $isAdmin || ($permissions->hide_text_to_speech ?? 0) != 1;
    $adminLongArticleAccess = $isAdmin || ($permissions->hide_long_article ?? 0) != 1;
    $adminChatAccess = $isAdmin || ($permissions->hide_chat ?? 0) != 1;
    $adminAiChatbotAccess = $isAdmin || ($permissions->hide_aichatbot ?? 0) != 1;
    $adminPlagiarismAccess = $isAdmin || ($permissions->hide_plagiarism ?? 0) != 1;
    $adminAiDetectorAccess = $isAdmin || ($permissions->hide_ai_detector ?? 0) != 1;
...
<nav id="sidenav"
    ...
    <div class="sidebar-bg-white h-full py-3.5 flex flex-col">
        ...
        <div class="sidebar-links sidebar-accordion middle-sidebar-scroll overflow-y-scroll">
            <ul class="mt-3">
                @if($adminTemplateAccess || $adminSpeechToTextAccess || $adminTextToSpeechAccess || $adminImageAccess || $adminCodeAccess || $adminLongArticleAccess || $adminChatAccess || $adminAiChatbotAccess || $adminPlagiarismAccess || $adminAiDetectorAccess)
                ...
                @if ($adminImageAccess)
                <li>
                    @php $menu = activeMenu(route('user.image.create')) @endphp
                    @if(customerPanelAccess('image'))
                    <a href="{{ route('user.image.create') }}">
                        <div
                            class="{{ $menu['class'] }} main-menu flex items-center gap-3 w-full py-3 pl-5 pr-[15px] text-left text-color-14 text-base leading-6 font-normal menus-height">
                            <span class="w-5 h-5">
                ...
                @php $chatbot = \Modules\Addons\Entities\Addon::find('Chatbot'); @endphp
                @if ($adminAiChatbotAccess && ($chatbot && $chatbot->isEnabled()))
                    <li>
                        @php $menu = activeMenu(route('aichatbot.index')) @endphp
                        <a href="{{ route('aichatbot.index') }}">
                            <div
                                class="{{ $menu['class'] }} main-menu flex items-center gap-3 w-full py-3 pl-5 pr-[15px] text-left text-color-14 text-base leading-6 font-normal menus-height">
                                ...
                                <p class="transion-hide accordion-menus">
                                    <span class="dark:text-white">{{ __('Ai Chatbot') }}</span>
                                </p>
                            </div>
                        </a>
                    </li>
                ...
                @if ($adminAiDetectorAccess)
                    <li>
                        @php $menu = activeMenu(route('user.aiDetectorTemplate')) @endphp
                        <a href="{{ route('user.aiDetectorTemplate') }}">
                            <div
                                class="{{ $menu['class'] }} main-menu flex items-center gap-3 w-full py-3 pl-5 pr-[15px] text-left text-color-14 text-base leading-6 font-normal menus-height">
                                <svg id="glipy_copy" viewBox="0 0 64 64" width="22" height="22" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" data-name="glipy copy">
                                ...
                                <p class="transion-hide accordion-menus">
                                    <span class="dark:text-white">{{ __('Ai Detector') }}</span>
                                </p>
                            </div>
                        </a>
                    </li>
                ...
                @if($adminTemplateAccess || $adminSpeechToTextAccess || $adminTextToSpeechAccess || $adminImageAccess || $adminCodeAccess || $adminLongArticleAccess || $adminChatAccess || $adminAiChatbotAccess || $adminPlagiarismAccess)
                <li class="w-[52px] div-border border dark:border-[#474746] border-t border-color-DF ml-5 my-3.5">
                </li>
                @endif
                @if ($adminTemplateAccess || $adminSpeechToTextAccess || $adminTextToSpeechAccess || $adminCodeAccess || $adminLongArticleAccess)
                <li>
                    @php 
                ...
                @if ($adminImageAccess)
                <li>
                    @php $menu = activeMenu(route('user.gallery.show'), route('user.image.index'))  @endphp
